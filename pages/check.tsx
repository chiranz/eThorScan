...
export default function check({}: Props): ReactElement {
  // getCode returns 0x if it is a address else returns the contract bytecode
  ...
  const handleSearch = async (address: string) => {
    setLoading(true);
    try {
      const _bal = await provider.getCode(address);
